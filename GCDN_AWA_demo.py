...
# ==============================
print ('Load data ...')
mat = scipy.io.loadmat('datasets/awa_data_VGG_ori.mat')
Xl=mat['Xl'] 
Xt=mat['Xu']
Yl=mat['Sl']
Yt=mat['Su']
...
# Set parameters
# ==============================
Z_dim=100 # Noise dimension for conditional GAN model
...
# Define placeholders
# ====================
X = tf.placeholder(tf.float32, shape=[None, feature_dim]) # Input feature matrix
y = tf.placeholder(tf.float32, shape=[None, label_dim]) # Input label matrix
Z = tf.placeholder(tf.float32, shape=[None, Z_dim]) # Input noise for 
...
def classifier(X):
    ...
    C_prob_100 = 100*C_prob # initial prediction from C_M(.) AWA dataset label range [0,100]
    
    # Define Correlation Discovery Network C_CDN(.)
    C_prob_1 = tf.expand_dims(C_prob, -1)
    C_prob_2 = tf.expand_dims(C_prob, 1)
    W_feature = tf.matmul(C_prob_1, C_prob_2) # build the relation matrix
    W_feature_1 = tf.reshape(W_feature, [-1, label_dim*label_dim])
    C_h2 = tf.nn.relu(tf.matmul(W_feature_1, C_W3) + C_b3)
    C_feature = tf.nn.sigmoid(tf.matmul(C_h2, C_W4) + C_b4)
    C_feature_100 = 100*C_feature # final output from C_CDN(.) AWA dataset label range [0,100]
    return C_prob_100, C_feature_100
...
D_solver = tf.train.AdamOptimizer(learning_rate=0.00005).minimize(D_loss, var_list=theta_D)
...
G_loss_sum = G_loss_dis + 1*G_loss_fea
G_solver = tf.train.AdamOptimizer(learning_rate=0.001).minimize(G_loss_sum, var_list=theta_G)

# Pre-trained generator loss
p_loss = tf.reduce_mean(tf.square(X-G_sample)) # initial the generator weights
p_solver = tf.train.AdamOptimizer().minimize(p_loss, var_list=theta_G)

# Classification loss of C_M(.)
GGG = 1
C_loss_cls_G_prob = tf.reduce_mean(tf.square(y-classified_label_G_prob)) # classification loss for generated samples
C_loss_cls_X_prob = tf.reduce_mean(tf.square(y-classified_label_X_prob)) # classification loss for real samples
C_loss_sum_prob = C_loss_cls_X_prob + GGG*C_loss_cls_G_prob
...
C_solver_sum = tf.train.AdamOptimizer(learning_rate=0.000003).minimize(C_loss, var_list=theta_C)
...
for seperate_train in range(300):
    for it in range(100):
        # obtain the training batch
        rand_idx = random.sample(range(data_number_labeled),mb_size) 
        X_mb = Xl[rand_idx,:]
        y_mb = Yl[rand_idx,:]
        Z_sample = 20*sample_Z(mb_size, Z_dim)
    ...
    if seperate_train % 1 == 0:
        cls_label_Ft = sess.run(classified_label_X_prob, feed_dict={X:Xt})        
        # =======================
        # Evaluation
